...
inputLines.forEach(currentInputLine => {
  if (currentInputLine==="" && moving===false){
    ...
    if(moving===false){
      ...
      const movingCrates=[]
      const testline='move 3 from 1 to 3'
      //const testline='move 2 from 7 to 2'
      if (currentInputLine===testline)console.log(testline,stacks,cratesToMove,stackFrom,stackTo)
      for(let i=cratesToMove;i>0;i--){
        const crate=stacks[stackFrom].pop()        
        if (currentInputLine===testline)console.log(crate)
        movingCrates.push(crate)
      }
      if (currentInputLine===testline)console.log(testline,stacks,movingCrates)
      
      for (let i=0;i<cratesToMove;i++){
        const crate=movingCrates.pop()     
        
        stacks[stackTo].push(crate)
        if (currentInputLine===testline)console.log(testline,crate,stacks,cratesToMove,stackFrom,stackTo)
      }
    }
