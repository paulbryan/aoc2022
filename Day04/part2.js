...
inputLines.forEach(currentInputLine => {
  if (currentInputLine===""){
   ...
   const efl1Array=getArray(elf1Min,elf1Max)
   const efl2Array=getArray(elf2Min,elf2Max)
    // console.log(elf1Min,elf1Max,efl1Array)
    // console.log(elf2Min,elf2Max,efl2Array)
    console.log(efl1Array,efl2Array,efl1Array.filter((item)=>{return efl2Array.indexOf(item)>=0}))
    if (efl1Array.filter((item)=>{return efl2Array.indexOf(item)>=0}).length>0){
      commonItems+=1
    }
  //  if (elf1Max>=elf2Min){
  //   commonItems+=1
